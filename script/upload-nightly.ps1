...
$bucketName = "hawk-nightly-host"
$releaseVersion = & "$PSScriptRoot\get-crate-version.ps1" hawk
$version = "$releaseVersion+nightly.$env:GITHUB_RUN_NUMBER.$env:GITHUB_SHA"

$remoteServerFiles = Get-ChildItem -Path "target" -Filter "hawk-remote-server-windows-*.zip" -Recurse -File -ErrorAction SilentlyContinue
...
UploadToBlobStore -BucketName $bucketName -FileToUpload "target/Hawk-$Architecture.exe" -BlobStoreKey "nightly/Hawk-$Architecture.exe"
UploadToBlobStore -BucketName $bucketName -FileToUpload "target/Hawk-$Architecture.exe" -BlobStoreKey "$version/Hawk-$Architecture.exe"

Remove-Item -Path "target/Hawk-$Architecture.exe" -ErrorAction SilentlyContinue
