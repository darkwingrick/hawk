...
function getHawkDataDir() {
  const platform = process.platform;

  if (platform === "darwin") {
    // macOS: ~/Library/Application Support/Hawk
    return path.join(os.homedir(), "Library", "Application Support", "Hawk");
  } else if (platform === "linux" || platform === "freebsd") {
    // Linux/FreeBSD: $FLATPAK_XDG_DATA_HOME or XDG_DATA_HOME/hawk
    if (process.env.FLATPAK_XDG_DATA_HOME) {
      return path.join(process.env.FLATPAK_XDG_DATA_HOME, "hawk");
    }
    const xdgDataHome = process.env.XDG_DATA_HOME || path.join(os.homedir(), ".local", "share");
    return path.join(xdgDataHome, "hawk");
  } else if (platform === "win32") {
    // Windows: LocalAppData/Hawk
    const localAppData = process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local");
    return path.join(localAppData, "Hawk");
  } else {
    // Fallback to XDG config dir
    const xdgConfigHome = process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config");
    return path.join(xdgConfigHome, "hawk");
  }
}
...
function writeBuildTimingJson(filePath, durationMs, firstCrate, target, blockedMs, command) {
  const buildTimingsDir = path.join(getHawkDataDir(), "build_timings");
