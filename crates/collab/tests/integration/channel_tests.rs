...
async fn test_channel_room(
    ...
    active_call_a
        .update(cx_a, |active_call, cx| active_call.join_channel(hawk_id, cx))
        .await
        .unwrap();
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(
                channels.channel_participants(hawk_id),
                &[client_a.user_id().unwrap()],
            );
        })
    });
    ...
    active_call_b
        .update(cx_b, |active_call, cx| active_call.join_channel(hawk_id, cx))
        .await
        .unwrap();
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(channels.channel_participants(hawk_id), &[]);
        })
    });
...
async fn test_channel_jumping(executor: BackgroundExecutor, cx_a: &mut TestAppContext) {
    ...
    let hawk_id = server
        .make_channel("hawk", None, (&client_a, cx_a), &mut [])
        .await;
    let rust_id = server
...
async fn test_channel_link_notifications(
    ...
    let channels = server
        .make_channel_tree(&[("hawk", None)], (&client_a, cx_a))
        .await;
    let hawk_channel = channels[0];

    try_join_all(client_a.channel_store().update(cx_a, |channel_store, cx| {
        [
            channel_store.set_channel_visibility(hawk_channel, proto::ChannelVisibility::Public, cx),
            channel_store.invite_member(hawk_channel, user_b, proto::ChannelRole::Member, cx),
            channel_store.invite_member(hawk_channel, user_c, proto::ChannelRole::Guest, cx),
        ]
    }))
    .await
    .unwrap();
    ...
    client_b
        .channel_store()
        .update(cx_b, |channel_store, cx| {
            channel_store.respond_to_channel_invite(hawk_channel, true, cx)
        })
        .await
        .unwrap();
    ...
    let active_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("active", Some(hawk_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(hawk_channel, 0), (active_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(client_c.channel_store(), cx_c, &[(hawk_channel, 0)]);

    let vim_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("vim", Some(hawk_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(hawk_channel, 0), (active_channel, 1), (vim_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(
        ...
        cx_c,
        &[(hawk_channel, 0), (vim_channel, 1)],
    );

    let helix_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("helix", Some(hawk_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_c.channel_store(),
        cx_c,
        &[(hawk_channel, 0), (vim_channel, 1), (helix_channel, 2)],
    );
}
...
async fn test_channel_membership_notifications(
    ...
    let channels = server
        .make_channel_tree(
            &[("hawk", None), ("vim", Some("hawk")), ("opensource", None)],
            (&client_a, cx_a),
        )
        .await;
    let hawk_channel = channels[0];
    ...
    try_join_all(client_a.channel_store().update(cx_a, |channel_store, cx| {
        [
            channel_store.set_channel_visibility(hawk_channel, proto::ChannelVisibility::Public, cx),
            channel_store.set_channel_visibility(vim_channel, proto::ChannelVisibility::Public, cx),
            channel_store.invite_member(hawk_channel, user_b, proto::ChannelRole::Admin, cx),
    ...
    client_b.channel_store().update(cx_b, |channel_store, _| {
        channel_store.is_channel_admin(hawk_channel)
    });
