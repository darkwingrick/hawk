        ...
        cx.spawn(async move |cx| {
            ...
            let task_templates = if is_package_json {
                ...
                package_json
                    ...
                    .chain([TaskTemplate {
                        label: "package script $HAWK_CUSTOM_script".to_owned(),
                        command: command.clone(),
                        args: vec![
                            "run".into(),
                ...
                serde_json_lenient::Value::from_str(&contents.text)
                    ...
                    .map(|key| TaskTemplate {
                        ...
                        args: vec!["-d".into(), "$HAWK_DIRNAME".into(), key.into()],
                        ..TaskTemplate::default()
                    })
                    .chain([TaskTemplate {
                        label: "composer script $HAWK_CUSTOM_script".to_owned(),
                        command: "composer".to_owned(),
                        args: vec![
                            "-d".into(),
                            "$HAWK_DIRNAME".into(),
...
impl LspAdapter for JsonLspAdapter {
    ...
    fn is_primary_hawk_json_schema_adapter(&self) -> bool {
        true
    }
}
