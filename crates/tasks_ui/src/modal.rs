...
pub use hawk_actions::{Rerun, Spawn};
        ...
        fs.insert_tree(
            ...
            json!({
                ".zed": {
                    "tasks.json": r#"[
                        {
                            "label": "hello from $HAWK_FILE:$HAWK_ROW:$HAWK_COLUMN",
                            "command": "echo",
                            "args": ["hello", "from", "$HAWK_FILE", ":", "$HAWK_ROW", ":", "$HAWK_COLUMN"]
                        },
                        {
                            "label": "opened now: $HAWK_WORKTREE_ROOT",
                            "command": "echo",
                            "args": ["opened", "now:", "$HAWK_WORKTREE_ROOT"]
                        }
                    ]"#,
                },
            ...
            language_registry.add(Arc::new(
                ...
                .with_context_provider(Some(Arc::new(
                    ContextProviderWithTasks::new(TaskTemplates(vec![
                        ...
                        TaskTemplate {
                            label: "TypeScript task from file $HAWK_FILE".to_string(),
                            command: "npm run build".to_string(),
                            ..TaskTemplate::default()
                        },
                        TaskTemplate {
                            label: "Another task from file $HAWK_FILE".to_string(),
