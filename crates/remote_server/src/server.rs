        ...
        Commands::Version => {
            ...
            match release_channel {
                ReleaseChannel::Stable | ReleaseChannel::Preview => {
                    println!("{}", env!("HAWK_PKG_VERSION"))
                }
                ReleaseChannel::Nightly | ReleaseChannel::Dev => {
                    let commit_sha =
                        option_env!("HAWK_COMMIT_SHA").unwrap_or(release_channel.dev_name());
                    let build_id = option_env!("HAWK_BUILD_ID");
...
pub static VERSION: LazyLock<String> = LazyLock::new(|| match *RELEASE_CHANNEL {
    ReleaseChannel::Stable | ReleaseChannel::Preview => env!("HAWK_PKG_VERSION").to_owned(),
    ReleaseChannel::Nightly | ReleaseChannel::Dev => {
        let commit_sha = option_env!("HAWK_COMMIT_SHA").unwrap_or("missing-zed-commit-sha");
        let build_identifier = option_env!("HAWK_BUILD_ID");
...
pub fn execute_run(
    ...
    crashes::init(
        crashes::InitCrashHandler {
            session_id: id,
            hawk_version: VERSION.to_owned(),
            binary: "zed-remote-server".to_string(),
            release_channel: release_channel::RELEASE_CHANNEL_NAME.clone(),
            commit_sha: option_env!("HAWK_COMMIT_SHA")
                .unwrap_or("no_sha")
                .to_owned(),
        },
        |task| {
    ...
    let run = move |cx: &mut _| {
        settings::init(cx);
        let app_commit_sha =
            option_env!("HAWK_COMMIT_SHA").map(|s| AppCommitSha::new(s.to_owned()));
...
fn is_new_version(version: &str) -> bool {
    semver::Version::from_str(version)
        .ok()
        .zip(semver::Version::from_str(env!("HAWK_PKG_VERSION")).ok())
