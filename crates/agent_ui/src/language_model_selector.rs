...
mod tests {
    ...
    fn test_exact_match(cx: &mut TestAppContext) {
        let models = create_models(vec![
            ("hawk", "Claude 3.7 Sonnet"),
            ("hawk", "Claude 3.7 Sonnet Thinking"),
            ("hawk", "gpt-5"),
            ("hawk", "gpt-5-mini"),
    ...
    fn test_fuzzy_match(cx: &mut TestAppContext) {
        ...
        assert_models_eq(results, vec!["hawk/Claude 3.7 Sonnet Thinking"]);
    }

    #[gpui::test]
    fn test_recommended_models_also_appear_in_other(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("hawk", "claude")]);
        let all_models = create_models(vec![
            ("hawk", "claude"), // Should also appear in "other"
            ("hawk", "gemini"),
            ("copilot", "o3"),
        ]);
    ...
    fn test_favorites_section_appears_when_favorites_exist(_cx: &mut TestAppContext) {
        ...
        let all_models = create_models_with_favorites(
            vec![("hawk", "claude"), ("hawk", "gemini"), ("openai", "gpt-4")],
            vec![("hawk", "gemini")],
        );
        ...
        assert_models_eq(grouped_models.favorites, vec!["hawk/gemini"]);
    }

    #[gpui::test]
    fn test_no_favorites_section_when_no_favorites(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("hawk", "claude")]);
        let all_models = create_models(vec![("hawk", "claude"), ("hawk", "gemini")]);
    ...
    fn test_models_have_correct_actions(_cx: &mut TestAppContext) {
        let recommended_models =
            create_models_with_favorites(vec![("hawk", "claude")], vec![("hawk", "claude")]);
        let all_models = create_models_with_favorites(
            vec![("hawk", "claude"), ("hawk", "gemini"), ("openai", "gpt-4")],
            vec![("hawk", "claude")],
        );
    ...
    fn test_favorites_appear_in_other_sections(_cx: &mut TestAppContext) {
        let favorites = vec![("hawk", "gemini"), ("openai", "gpt-4")];

        let recommended_models =
            create_models_with_favorites(vec![("hawk", "claude")], favorites.clone());
