...
// These environment variables all use a `HAWK_` prefix because we don't want to overwrite the user's AWS credentials.
static HAWK_BEDROCK_ACCESS_KEY_ID_VAR: LazyLock<EnvVar> = env_var!("HAWK_ACCESS_KEY_ID");
static HAWK_BEDROCK_SECRET_ACCESS_KEY_VAR: LazyLock<EnvVar> = env_var!("HAWK_SECRET_ACCESS_KEY");
static HAWK_BEDROCK_SESSION_TOKEN_VAR: LazyLock<EnvVar> = env_var!("HAWK_SESSION_TOKEN");
static HAWK_AWS_PROFILE_VAR: LazyLock<EnvVar> = env_var!("HAWK_AWS_PROFILE");
static HAWK_BEDROCK_REGION_VAR: LazyLock<EnvVar> = env_var!("HAWK_AWS_REGION");
static HAWK_AWS_ENDPOINT_VAR: LazyLock<EnvVar> = env_var!("HAWK_AWS_ENDPOINT");
static HAWK_BEDROCK_BEARER_TOKEN_VAR: LazyLock<EnvVar> = env_var!("HAWK_BEDROCK_BEARER_TOKEN");
...
impl State {
    ...
    fn load_static_credentials(
        ...
        cx.spawn(async move |this, cx| {
            ...
            } else if let Some(access_key_id) = &HAWK_BEDROCK_ACCESS_KEY_ID_VAR.value {
                if let Some(secret_access_key) = &HAWK_BEDROCK_SECRET_ACCESS_KEY_VAR.value {
                    if !access_key_id.is_empty() && !secret_access_key.is_empty() {
                        let session_token = HAWK_BEDROCK_SESSION_TOKEN_VAR
                            .value
                            .as_deref()
                            .filter(|s| !s.is_empty())
                            .map(|s| s.to_string());
    ...
    fn get_region(&self) -> String {
        ...
        if let Some(region) = HAWK_BEDROCK_REGION_VAR.value.as_deref() {
            if !region.is_empty() {
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let configured_label = match &auth {
            ...
            Some(BedrockAuth::IamCredentials { .. }) if env_var_set => {
                format!(
                    ...
                    HAWK_BEDROCK_ACCESS_KEY_ID_VAR.name, HAWK_BEDROCK_SECRET_ACCESS_KEY_VAR.name
                )
            }
            ...
            Some(BedrockAuth::ApiKey { .. }) if env_var_set => {
                format!(
                    ...
                    HAWK_BEDROCK_BEARER_TOKEN_VAR.name
                )
            }
        ...
        let tooltip_label = if env_var_set {
            Some(format!(
                ...
                HAWK_BEDROCK_SESSION_TOKEN_VAR.name,
                HAWK_BEDROCK_BEARER_TOKEN_VAR.name
            ))
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    ...
                    HAWK_AWS_PROFILE_VAR.name,
                    HAWK_AWS_ENDPOINT_VAR.name,
                    HAWK_BEDROCK_SESSION_TOKEN_VAR.name
                ))
                .size(LabelSize::Small)
                .color(Color::Muted)
