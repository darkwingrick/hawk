...
#[cfg(windows)]
impl MasterProcess {
    const CONNECTION_ESTABLISHED_MAGIC: &str = "HAWK_SSH_CONNECTION_ESTABLISHED";
...
impl RemoteConnection for SshRemoteConnection {
    ...
    fn start_proxy(
        ...
        const VARS: [&str; 3] = ["RUST_LOG", "RUST_BACKTRACE", "HAWK_GENERATE_MINIDUMPS"];
    ...
    async fn platform_windows(&self, shell: ShellKind) -> Result<RemotePlatform> {
        ...
        Ok(RemotePlatform {
            ...
            arch: match output.trim() {
                ...
                arch => anyhow::bail!(
                    "Prebuilt remote servers are not yet available for windows-{arch}. See https://hawk.dev/docs/remote-development"
                ),
            },
        })
    }
