...
use edit_prediction::{EditPredictionModel, HawkEditPredictionDelegate, Zeta2FeatureFlag};
use editor::Editor;
...
fn edit_prediction_provider_config_for_settings(cx: &App) -> Option<EditPredictionProviderConfig> {
    ...
    match provider {
        ...
        EditPredictionProvider::Hawk => Some(EditPredictionProviderConfig::Zed(
...
fn assign_edit_prediction_provider(
    ...
    match provider_config {
        None => {
            editor.set_edit_prediction_provider::<HawkEditPredictionDelegate>(None, window, cx);
        }
            ...
            if let Some(project) = editor.project() {
                ...
                if has_model {
                    let provider = cx.new(|cx| {
                        HawkEditPredictionDelegate::new(
                            project.clone(),
                            singleton_buffer,
                            &client,
                            &user_store,
