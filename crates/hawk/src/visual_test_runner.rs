...
//!   cargo run -p zed --bin hawk_visual_test_runner --features visual-tests
//!
//! Update baseline images (when UI intentionally changes):
//!   UPDATE_BASELINE=1 cargo run -p zed --bin hawk_visual_test_runner --features visual-tests
//!
...
#[cfg(target_os = "macos")]
fn main() {
    // Set HAWK_STATELESS early to prevent file system access to real config directories
    // This must be done before any code accesses hawk_env_vars::HAWK_STATELESS
    // SAFETY: We're at the start of main(), before any threads are spawned
    unsafe {
        std::env::set_var("HAWK_STATELESS", "1");
    }
...
fn run_multi_workspace_sidebar_visual_tests(
    ...
    let workspace2_dir = canonical_temp.join("hawk");
    ...
    // Add worktree to workspace 2 (index 1) so it shows as "hawk"
