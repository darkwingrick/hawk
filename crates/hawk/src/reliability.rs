...
async fn upload_minidump(
    ...
    let mut form = Form::new()
        ...
        .text("sentry[tags][version]", metadata.init.hawk_version.clone())
    ...
    ::telemetry::event!(
        ...
        crashed_version = metadata.init.hawk_version.clone(),
    ...
    for (index, gpu) in metadata.gpus.iter().cloned().enumerate() {
        ...
        form = form
            .text(
                ...
                "A GPU found on the users system. May or may not be the GPU Hawk is running on",
            )
