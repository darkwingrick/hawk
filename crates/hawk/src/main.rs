...
use client::{Client, ProxySettings, UserStore, parse_hawk_link};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Hawk failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Hawk failed to open a window: {e:?}. See https://github.com/darkwingrick/hawk/tree/master/docs for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.hawk.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Hawk failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://github.com/darkwingrick/hawk/tree/master/docs for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    // `hawk --askpass` Makes zed operate in nc/netcat mode for use with askpass
    ...
    // `hawk --crash-handler` Makes zed operate in minidump crash handler mode
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) =
            etw_tracing::record_etw_trace(hawk_pid, &output_path, etw_socket.as_str())
        {
    ...
    // `hawk --nc` Makes zed operate in nc/netcat mode for use with MCP
    ...
    // `hawk --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    #[cfg(target_os = "windows")]
    match util::get_hawk_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("HAWK_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    let version = option_env!("HAWK_BUILD_ID");
    let app_commit_sha =
        option_env!("HAWK_COMMIT_SHA").map(|commit_sha| AppCommitSha::new(commit_sha.to_string()));
    ...
    if args.system_specs {
        ...
        println!("Hawk System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    if feature_flags::CrashReportingFeatureFlag::enabled_for_all() {
        crashes::init(
            InitCrashHandler {
                session_id,
                hawk_version: app_version.to_string(),
                binary: "hawk".to_string(),
    ...
    let failed_single_instance_check = if *hawk_env_vars::HAWK_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::hawk::listen_for_cli_connections(open_listener.clone()).is_err()
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use hawk::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("hawk is already running");
        return;
    }

    let git_hosting_provider_registry = Arc::new(GitHostingProviderRegistry::new());
    let git_binary_path =
        if cfg!(target_os = "macos") && option_env!("HAWK_BUNDLE").as_deref() == Some("true") {
    ...
    app.run(move |cx| {
        ...
        if feature_flags::TelemetryFeatureFlag::enabled_for_all() {
        ...
        acp_tools::init(cx);
        hawk::telemetry_log::init(cx);
        hawk::remote_debug::init(cx);
        ...
        client.telemetry().flush_events().detach();
    ...
    if let Some(kind) = request.kind {
        match kind {
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("hawk://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // hawk://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // hawk://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(hawk_actions::OpenSettings), cx),
                        Some(setting_path) => window.dispatch_action(
                            Box::new(hawk_actions::OpenSettingsAt { path: setting_path }),
                            cx,
                        ),
                    })
                })
...
#[derive(Parser, Debug)]
#[command(name = "hawk", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `hawk://` scheme, or relative to <https://hawk.dev>.
    ...
    /// On macOS, the default is `~/Library/Application Support/Hawk`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/hawk`.
    /// On Windows, the default is `%LOCALAPPDATA%\Hawk`.
    ...
    /// Instructs hawk to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Hawk from starting, so you can't run `hawk: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for the MCP Server, to remove the need for netcat as a dependency,
    /// by having Hawk act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
    nc: Option<String>,

    /// Used for recording minidumps on crashes by having Hawk run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run hawk in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// The PID of the Hawk process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true, allow_hyphen_values = true)]
    etw_hawk_pid: Option<i64>,
    ...
    /// Unix socket path for IPC with the parent Hawk process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("hawk://")
                || arg.starts_with("hawk-cli://")
                || arg.starts_with("ssh://")
                || parse_hawk_link(arg, cx).is_some()
            {
                arg.into()
            } else {
                format!("file://{arg}")
