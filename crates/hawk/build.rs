...
fn main() {
        ...
        && output.status.success()
    {
            ...
            && build_profile == "release"
        {
            ...
            println!("cargo::warning=Info: using '{git_sha}' hash for HAWK_COMMIT_SHA env var");
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(explicit_rc_toolkit_path) = std::env::var("HAWK_RC_TOOLKIT_PATH").ok() {
            res.set_toolkit_path(explicit_rc_toolkit_path.as_str());
        }
        res.set_icon(icon.to_str().unwrap());
        res.set("FileDescription", "Hawk");
        res.set("ProductName", "Hawk");
