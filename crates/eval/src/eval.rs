...
fn main() {
    ...
    let hawk_commit_sha = commit_sha_for_path(&root_dir);
    let hawk_branch_name = git_branch_for_path(&root_dir);
    ...
    app.run(move |cx| {
        ...
        let enable_telemetry = env::var("HAWK_EVAL_TELEMETRY").is_ok_and(|value| value == "1")
            && telemetry.has_checksum_seed();
        if enable_telemetry {
            println!("Telemetry enabled");
            telemetry::event!(
                "Agent Eval Started",
                hawk_commit_sha = hawk_commit_sha,
                hawk_branch_name = hawk_branch_name,
                run_id = run_id,
            );
        }
        ...
        cx.spawn(async move |cx| {
            ...
            future::join_all((0..args.concurrency).map(|_| {
                ...
                let hawk_commit_sha = hawk_commit_sha.clone();
                let hawk_branch_name = hawk_branch_name.clone();
