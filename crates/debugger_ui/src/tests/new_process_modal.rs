...
async fn test_debug_session_substitutes_variables_and_relativizes_paths(
    ...
    let test_cases: Vec<(&'static str, &'static str)> = vec![
        ...
        // Path with $HAWK_WORKTREE_ROOT - should be substituted without double appending
        (
            format!(
                "$HAWK_WORKTREE_ROOT{0}src{0}program",
            ...
            move |client| {
                client.on_request::<dap::requests::Launch, _>({
                    ...
                    move |_, args| {
                        ...
                        let expected_other_field = if input_path.contains("$HAWK_WORKTREE_ROOT") {
                            input_path.replace("$HAWK_WORKTREE_ROOT", path!("/test/worktree/path"))
                        } else {
...
async fn test_dap_adapter_config_conversion_and_validation(cx: &mut TestAppContext) {
    ...
    for adapter_name in adapter_names {
        ...
        let mut adapter_specific_config = hawk_config.clone();
        adapter_specific_config.adapter = adapter_name.to_string().into();

        let debug_scenario = adapter
            .config_from_hawk_format(adapter_specific_config)
            .await
            .unwrap_or_else(|_| {
                panic!(
