...
/// `.hawk/settings.json` where `.hawk` is a symlink to an external directory.
///
/// Note: intermediate directories *can* be symlinks (not just leaf entries),
/// so we must walk the full ancestor chain. For example:
///   `ln -s /external/config /project/.hawk`
/// makes `.hawk` an intermediate symlink directory.
...
/// either inside a `.hawk/` local-settings directory or inside the global config dir.
