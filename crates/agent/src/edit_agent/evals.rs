...
impl EditAgentTest {
    async fn new(cx: &mut TestAppContext) -> Self {
        ...
        let agent_model = SelectedModel::from_str(
            &std::env::var("HAWK_AGENT_MODEL").unwrap_or("anthropic/claude-sonnet-4-latest".into()),
        )
        .unwrap();
        let judge_model = SelectedModel::from_str(
            &std::env::var("HAWK_JUDGE_MODEL").unwrap_or("anthropic/claude-sonnet-4-latest".into()),
        )
        .unwrap();
