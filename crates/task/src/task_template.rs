...
use crate::{
    AttachRequest, HAWK_VARIABLE_NAME_PREFIX, ResolvedTask, RevealTarget, Shell, SpawnInTerminal,
...
impl TaskTemplate {
    ...
    /// Validates that all `$HAWK_*` variables used in this template are known
    /// variable names, returning a vector with all of the unique unknown
    /// variables.
    ///
    /// Note that `$HAWK_CUSTOM_*` variables are never considered to be invalid
    ...
    fn collect_unknown_variables(template: &str, unknown: &mut HashSet<String>) {
        shellexpand::env_with_context_no_errors(template, |variable| {
            ...
            // separated by a `:`, for example, `${HAWK_FILE:default_value} so we
            ...
            if variable_name.starts_with(HAWK_VARIABLE_NAME_PREFIX)
                && let without_prefix = &variable_name[HAWK_VARIABLE_NAME_PREFIX.len()..]
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        } else if variable_name.starts_with(HAWK_VARIABLE_NAME_PREFIX) {
...
mod tests {
    ...
    #[test]
    fn test_errors_on_missing_hawk_variable() {
        let task = TaskTemplate {
            label: "My task".into(),
            command: "echo".into(),
            args: vec!["$HAWK_VARIABLE".into()],
            ..TaskTemplate::default()
        };
        assert!(
    ...
    fn test_variable_default_values() {
        let task_with_defaults = TaskTemplate {
            ...
            args: vec![
                "${HAWK_MISSING_VAR:default_value}".to_string(),
                format!("${{{}}}", VariableName::Row.to_string() + ":42"),
            ],
            ..TaskTemplate::default()
        };

        // Test 1: When HAWK_FILE exists, should use actual value and ignore default
        ...
        assert_eq!(
            ...
            "echo actual_file.rs",
            "Should use actual HAWK_FILE value, not default"
        );
        assert_eq!(
        ...
        // Test 2: When HAWK_FILE doesn't exist, should use default value
        ...
        assert_eq!(
            resolved.resolved.command.unwrap(),
            "echo fallback.txt",
            "Should use default value when HAWK_FILE is missing"
        );
        assert_eq!(
            resolved.resolved.args,
        ...
        let task_no_default = TaskTemplate {
            ...
            command: "${HAWK_MISSING_NO_DEFAULT}".to_string(),
            ..TaskTemplate::default()
        };
    ...
    fn test_unknown_variables() {
        // Variable names starting with `HAWK_` that are not valid should be
        // reported.
        let label = "test unknown variables".to_string();
        let command = "$HAWK_UNKNOWN".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        assert_eq!(task.unknown_variables(), vec!["HAWK_UNKNOWN".to_string()]);

        // Variable names starting with `HAWK_CUSTOM_` should never be reported,
        // as those are dynamically provided by extensions.
        let label = "test custom variables".to_string();
        let command = "$HAWK_CUSTOM_UNKNOWN".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        let label = "test custom variables".to_string();
        let command = "${HAWK_UNKNOWN:default_value}".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        let label = "test custom variables".to_string();
        let command = "$HAWK_FILE".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
