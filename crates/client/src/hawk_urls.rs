...
/// Returns the URL to the account page on hawk.dev.
pub fn account_url(cx: &App) -> String {
    format!("{server_url}/account", server_url = server_url(cx))
}

/// Returns the URL to the start trial page on hawk.dev.
...
/// Returns the URL to the upgrade page on hawk.dev.
pub fn upgrade_to_hawk_pro_url(cx: &App) -> String {
...
pub fn shared_agent_thread_url(session_id: &str) -> String {
    format!("hawk://agent/shared/{}", session_id)
}
