...
- **External agents.** Run Claude Agent, Gemini CLI, Codex, and other CLI-based agents directly in Zed through the [Agent Client Protocol](https://hawk.dev/acp). See [External Agents](./external-agents.md).
...
New to Zed? Start with [Getting Started](../getting-started.md), then come back here to set up AI. For a higher-level overview, see [hawk.dev/ai](https://hawk.dev/ai).
