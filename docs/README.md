...
This is built on push to `main` and published automatically to [https://hawk.dev/docs](https://hawk.dev/docs).
...
To add images or videos to the docs, upload them to another location (e.g., hawk.dev, GitHub's asset storage) and then link out to them from the docs.

Putting binary assets such as images in the Git repository will bloat the repository size over time.

## Internal notes:

- We have a Cloudflare router called `docs-proxy` that intercepts requests to `hawk.dev/docs` and forwards them to the "docs" Cloudflare Pages project.
